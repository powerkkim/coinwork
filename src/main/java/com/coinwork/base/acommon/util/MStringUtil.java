...
public class MStringUtil {
    ...
    /**
     * <p>
     * String이 비었거나("") 혹은 null 일경우 default data를 넘겨주도록 처리.
     * </p>
     * @param str - str이 null 이거나 "" 빈 문자열이라면 기본값을 설정함.
     * @return <code>long</code> - 입력받은 String 이 값이 존재 한다면 해당값을 리턴하며, 값이 존재 하지 않는다면 기본값을 리턴.
     */
    public static String emptyDefaultString(String str, String defaultValue) throws Exception {
        String data;
        try {
            data = !isEmpty( str ) ? str : defaultValue;
        } catch (NumberFormatException e) {
            e.printStackTrace();
            throw new Exception( MessageCode.ERROR_TYPE_000.toString() );
        }
        return data;
    }
     ...
     * @return <code>long</code> - 입력받은 String 이 값이 존재 한다면 해당값을 리턴하며, 값이 존재 하지 않는다면 기본값을 리턴.
     */
    public static long emptyDefaultLong(String str, long defaultValue) throws Exception {
        long data;
        try {
            data = !isEmpty( str ) ? Long.parseLong( str ) : defaultValue;
        } catch (NumberFormatException e) {
            e.printStackTrace();
            throw new Exception( MessageCode.ERROR_TYPE_000.toString() );
        }
     ...
     * @return <code>long</code> - 입력받은 String 이 값이 존재 한다면 해당값을 리턴하며, 값이 존재 하지 않는다면 기본값을 리턴.
     */
    public static int emptyDefaultInt(String str, int defaultValue) throws Exception {
        int data;
        try {
            data = !isEmpty( str ) ? Integer.parseInt( str ) : defaultValue;
        } catch (NumberFormatException e) {
            e.printStackTrace();
            throw new Exception( MessageCode.ERROR_TYPE_000.toString() );
        }
     ...
     * @return String
     */
    public static String getRandomKeyLowerCase(int len) {
        String chararray = "abcdefghijklmnopqrstuvwxyz0123456789";
        String ret = "";
        for(int i = 0; i < len; i++) {
            ret += chararray.charAt((int)(Math.random()*100)%chararray.length());
     ...
     * @throws Exception
     */
    public static String getRandomKey() throws Exception{
    ...
    /**
     * 주어진 숫자의 길이 만큼 랜덤한 문자열(숫자)을 만들어 리턴.
     * @param len
     * @return
     * @throws Exception
     */
    public static String getRandomKeyNumber(int len) {
